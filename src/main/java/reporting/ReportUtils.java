...
public final class ReportUtils {
	...
	public static String createExtentReportPath() {
		...
		if (OVERRIDE_REPORTS.trim().equals(NO)) {
			...
			link = EXTENT_REPORT_FOLDER_PATH + "/" + DateUtils.getCurrentDate() + "_" + EXTENT_REPORT_FILE_NAME;
		...
		if (OPEN_REPORTS_AFTER_EXECUTION.trim().equalsIgnoreCase(YES)) {
			try {
				...
				throw new InvalidPathForExtentReportFileException(
						"Extent Report file you are trying to reach is not found",
						fileNotFoundException.fillInStackTrace());
			} catch (IOException ioException) {
				throw new FrameworkException(
						"Extent Report file you are trying to reach got IOException while reading the file",
